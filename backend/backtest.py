...
def run_backtest(candles: list[dict], strategy: str, initial_cash: float, fee_rate: float = 0.001,
                 slippage_rate: float = 0.0005, interval: str = "1h", asset_type: str = "crypto") -> dict:
    ...
    if strategy in {"ai","ai_technical"}:
        df,up_probability=out_of_sample_probabilities(candles,interval,asset_type);df["ai_up"]=up_probability
    signals=[]
    ...
    for (_,row),signal in zip(df.iterrows(),executable_signals):
    ...
    equities=np.asarray([point["equity"] for point in curve],dtype=float)
    returns=np.diff(equities)/np.maximum(equities[:-1],1e-12)
    bars_per_day={"1m":1440,"5m":288,"15m":96,"30m":48,"1h":24,"4h":6,"1d":1}[interval]
    periods=(365 if asset_type=="crypto" else 252)*bars_per_day
