...
当前正式版本 v1.15.0 新增 Trader Decision Desk：把自选资产的行情新鲜度、新闻、技术证据、失效线与状态变化聚合为本地持久化简报。页面先读本地快照、后台刷新外部数据；过期数据会关闭决策门，失效线越界与高影响新闻会生成可复核提醒。真实审计仍为 `NO EDGE`，`Production Model = NONE`，不会将研究观察包装成买卖建议。
...
双击 `outputs\desktop\AI-Market-Assistant-Setup-v1.15.0.exe` 完成安装。桌面会出现“AI行情助手”，以后直接双击即可；无需打开项目目录、Python、Node、Docker 或命令行。
